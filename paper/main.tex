...
\thanks{O. Hassani is with ESILV Graduate School of Engineering, De Vinci Higher Education, Paris-La D\'efense, France.}}

\begin{document}
\maketitle

\begin{abstract}
Optimal execution is a sequential decision problem in which predictive signal, market microstructure, queue uncertainty, transaction costs, latency, and computational constraints interact. This paper develops an end-to-end research system for studying machine-learning-assisted execution in a limit-order-book environment. The system combines an exact integer-arithmetic C++ matching and replay engine, explicit aggregate-L2 queue models, classical execution baselines, a finite-horizon receding model-predictive controller (MPC), causal microstructure prediction, a compact Conv1D--LSTM temporal model, calibrated ML-assisted MPC, imitation learning with dataset aggregation, categorical proximal policy optimization (PPO), dependence-aware robustness statistics, and profile-driven CPU/GPU performance engineering.

The central experimental question is not whether a forecasting model can achieve a better standalone probability score, but whether forecast improvements survive the downstream optimization map and improve execution decisions. Controlled experiments expose several non-monotonic effects. Calibration can improve log loss while producing a weaker controller response; an oracle for an intermediate quote-depletion event can induce worse implementation shortfall; a compact imitation policy reproduces an MPC teacher in-distribution but exhibits a pronounced out-of-distribution fidelity loss; and PPO does not dominate strong deterministic execution heuristics. Across 43 registered stress conditions, the identity of the lowest-cost strategy changes in 16 of 42 non-central regimes. A paired moving-block bootstrap with 4,096 replications per contrast further shows that 85 of 129 confidence intervals cross zero and that 22 of 43 point-estimate stress winners have less than 80\% bootstrap probability of remaining best.

The systems study reaches an equally workload-dependent conclusion. A profile-guided C++ capacity optimization raises four-thread matching throughput to approximately 17.3 million operations/s on the measured CPU workload. For neural inference on a Tesla T4, transfer-inclusive GPU execution is slower than CPU for all registered batch-one decision workloads; the temporal model becomes GPU-favorable only at batch 256, where the measured transfer-inclusive speedup is approximately 1.70$\times$. These results support a broader methodological conclusion: in low-latency execution, predictive quality, decision quality, policy complexity, robustness, and hardware acceleration must be evaluated jointly rather than optimized as independent proxies.
\end{abstract}

\begin{IEEEkeywords}
optimal execution, limit order book, market microstructure, model predictive control, machine learning, deep learning, imitation learning, reinforcement learning, PPO, high-performance computing, CUDA, latency, robustness, block bootstrap.
\end{IEEEkeywords}
...
\begin{itemize}[leftmargin=*]
    \item an exact, deterministic C++ event, matching, replay, and execution-accounting core using integer ticks/lots and auditable event ordering;
    \item three explicit aggregate-L2 queue assumptions that expose queue-position uncertainty instead of silently treating displayed depth as exact FIFO state;
    \item a matched non-ML/ML MPC construction in which the optimizer, action space, queue model, latency model, and terminal accounting remain fixed while only a prediction-derived passive-risk term changes;
    \item a causal 20-feature microstructure contract, simple calibrated model ladder, and compact Conv1D--LSTM temporal model evaluated at 250~ms, 1~s, and 5~s horizons;
    \item a prediction-versus-decision ablation framework covering neutral, shuffled, stale, uncalibrated, oracle, and zero-weight controls;
    \item an imitation-learning pipeline with an exact C++ teacher oracle, a validation-triggered DAgger correction round, explicit OOD diagnostics, and conservative fallback;
    \item a multi-seed categorical PPO environment with independently reconstructable rewards and anti-reward-hacking tests;
    \item a 43-cell robustness matrix and dependence-aware paired block-bootstrap analysis with Holm multiplicity correction and ranking-stability probabilities;
    \item a profile-first systems study spanning C++ throughput, multithread scaling, Python/C++ boundary cost, compiled neural inference, and transfer-inclusive Tesla T4 CUDA timing.
\end{itemize}
...
\subsection{Experimental Scope and External Validity}
The controlled experiments are designed to isolate causal mechanisms and model-risk channels under reproducible state transitions. External market deployment introduces additional variation in hidden liquidity, exchange-specific matching behavior, network jitter, venue outages, impact endogeneity, and participant adaptation. The architecture addresses these factors through explicit latency, queue-model, impact, information-degradation, and simulator-mismatch stresses, but any new venue or historical regime should be treated as a new external-validity test rather than assumed to share the controlled distribution. This is a standard distinction between internal experimental identification and external market generalization.
...
The final native matrix contains 53 tests and is exercised under GCC Debug, Clang Debug, optimized GCC Release, and ASan/UBSan. The final Python inventory contains 478 tests: 477 are represented in the branch-coverage partitions and one additional full artifact-regeneration test is executed separately; combined branch-aware repository coverage is 91\%. Independent semantic validators check data contracts, feature causality, model artifacts, controller equivalence, imitation/RL reports, robustness statistics, and performance evidence. Expensive regeneration suites are run in partitions rather than hidden behind a monolithic command timeout.

The final source tree additionally provides CMake packaging, an external consumer check, Docker/reproduction entry points, JSON Schemas for stage outputs, checksum manifests, and a citation file. This structure follows the general reproducibility principle that a computational result should be accompanied by sufficiently precise code, configuration, seeds, and compute information to make regeneration auditable \cite{PineauVincentLamarreSinhaLariviere2021}.
...
\midrule
C++ native & GCC Debug / Clang Debug / GCC Release / ASan+UBSan & 53/53 each \\
Python & partitioned unit/integration suites & 478 total; 477 coverage + 1 full regeneration \\
Coverage & branch-aware combined repository measurement & 91\% \\
Specification & frozen-file hash lock & 7/7 \\
Repository contract & required-file/schema contract & 561/561 \\
